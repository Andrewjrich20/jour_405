...
md_cities_rates <- md_cities |>
  mutate(violent_rate_2019 = (violent2019/pop2019)*100,000,
         property_rate_2019 = (property2019/pop2019)*100,000,
         violent_rate_2020 = (violent2020/pop2020)*100,000,
         property_rate_2020 = (property2020/pop2020)*100,000)
```
...
ANSWER HERE: For both property crime and violent crime, Ocean City ranks within the top 2. Baltimore ranks number 1 for violent crime in both 2019 and 2020. The story here is that Baltimore has the highest violent crime rate but the property crime rate is considerably lower.
...
md_cities_change <- md_cities_rates |>
  mutate(violent_change = ((violent_rate_2019 - violent_rate_2020)/violent_rate_2019)*100,
         property_change = ((property_rate_2019 - property_rate_2020)/property_rate_2019)*100)
...
ANSWER HERE: The patterns are more interesting in this data. The outliers do not give a proper understanding of the crime rates. The story in this data is the Frederick has seen large percentage increases in their crime rate from 2019 to 2020, for both violent and property crimes.
...
ANSWER HERE: Frederick, MD has seen alarming increases in both violent and property crime rates from 2019 to 2020. There was a 95% increase in violent crime rate and a 90% increase in property crime rate in Frederick. Of the cities in Maryland that had a population over 2,000 people, Frederick had by far the biggest increase in their crime rate. 
...
md_cities_percentiles <- md_cities_change |>
  mutate(percentiles = ntile(violent_rate_2020, 100)) 
...
md_cities_percentiles |>
  summarise(IQR(violent_rate_2020))
```
This new information does change my best story idea from the previous assignment. This is because Frederick is only in the 22nd percentile for violent crime rates in Maryland. I do not think that warrants a story because of the percentile ranking.