...
```{r}
gdp |> summarise(correlation = cor(DJIA, GDP, method = "pearson"))
```
...
gdp |> 
  ...
  labs(title = "DJIA and GDP Correlation")
```
...
There is a clear correlation between the Dow Jones Industrial Average and the GDP of the US. Just because they are correlated doesn't mean that one causes the other but there is a relationship between the two economic indicators. The correlation is pretty strong because most of the points on the graph are near the linear line. However, the scatterplot does show that as DJIA and GDP get higher the correlation becomes weaker. 
